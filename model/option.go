...
func InitOptionMap() {
	...
	common.OptionMap["ModelSizePrice"] = ratio_setting.ModelSizePrice2JSONString()
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" {
		...
		case "QuotaBucketBillingEnabled":
			setting.QuotaBucketBillingEnabled = boolValue
	...
	case "ModelSizePrice":
		err = ratio_setting.UpdateModelSizePriceByJSONString(value)
	case "CacheRatio":
